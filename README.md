...
Anthropic VM から Discord に直接POSTすると DNS cache overflow が発生するため、Discord投稿は GitHub Actions に分離している。ルーティーンは作業ブランチにしか push できない（インフラ仕様）ため、main への反映は PR 経由とし、auto-merge も同じ Actions に統合している。
...
[docs\routine-setup.md](docs\routine-setup.md) を参照してください。
