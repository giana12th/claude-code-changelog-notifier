...
Routine（Anthropic VM）→ `pending/<routine>.txt` に書き出し → git push → GitHub Actions が push を検知して Discord に投稿 → 成功したら pending を削除。

Anthropic VM から Discord に直接POSTすると DNS cache overflow が発生するため、Discord投稿だけ GitHub Actions に分離している。
...
- **Read and write permissions** を選択（workflow が pending 削除 commit を push するため）

設定後、Actions タブに "Discord Notify" workflow が表示されることを確認する。
