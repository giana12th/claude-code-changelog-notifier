...
8. `pending/research.txt` と `state/last_research.txt` をまとめて git commit & push する
...
push 後は GitHub Actions が `pending/research.txt` を読んで Discord に投稿し、成功したらファイルを削除する。投稿成否はルーティーン側では関知しない。
